function update_visualization_func = show_video(img_files, video_path)
%SHOW_VIDEO
...
% 	set(fig_h, 'number','off', 'name', ['Tracker - ' video_path])
	...
	stop_tracker = false;
    im_fire = cell(num_frames,1);
	

	function stop = update_visualization(frame, box, firing_map)
        %, stS, avgs, sums, avgs2, sums2
		%store the tracker instance for one frame, and show it. returns
		%true if processing should stop (user pressed 'Esc').
        im_fire{frame} = firing_map;
	...
	function redraw(frame)
        ...
        im_fire2 = im_fire{frame};
		if(~isempty(im_fire2))
            im = cat(1, im, uint8(imresize(im_fire2, [round(4*size(im,2)/65), size(im,2)])*255));
        else
            im = cat(1, im, zeros([round(4*size(im,2)/65), size(im,2), 3]));
        end
